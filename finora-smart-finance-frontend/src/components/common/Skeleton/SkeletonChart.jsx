 ...
 * @description Für AdminCharts (Pie, Bar, Line)
 *
...
const SkeletonChart = memo(
  ({ variant = 'bar', hasTitle = true, hasLegend = true, height = '200px', className = '' }) => {
            ...
            <div className={styles.barContainer}>
              {[...Array(6)].map((_, i) => (
                <div key={i} className={styles.barColumn}>
                  <Skeleton width="100%" height={`${30 + Math.random() * 70}%`} variant="rect" />
                </div>
              ))}
            </div>
          );
